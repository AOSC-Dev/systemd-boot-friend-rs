...
info_path_not_exist =
    systemd-boot-friend 似乎尚未初始化。
    請執行 `systemd-boot-friend init` 即可
    安裝並設定 systemd-boot。
...
notice_init =
    systemd-boot-friend 即將安裝及初始化 systemd-boot，
    並將其設定為預設 EFI 開機選項。完成後，您依舊可以從
    EFI 開機管理程式中存取其他開機載入器，
...
prompt_update =
    systemd-boot 已成功初始化。是否要讓 systemd-boot-friend
    搜尋 `{ $src_path }` 中的內核，並將其登記至 systemd-boot 設定檔中？
...
current_bootarg = 偵測到目前的啟動引數（內核命令列）：
current_root = 偵測到目前的根分割區：{ $root }
